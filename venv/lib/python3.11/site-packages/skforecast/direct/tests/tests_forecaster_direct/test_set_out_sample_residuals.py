...
def test_set_out_sample_residuals_TypeError_when_y_pred_is_not_array_or_series():
    """
...
def test_set_out_sample_residuals_ValueError_when_y_true_and_y_pred_have_different_length():
    ...
    err_msg = re.escape(
        "`y_true` and `y_pred` must have the same length. Got 5 and 3."
    )
    with pytest.raises(ValueError, match = err_msg):
        forecaster.set_out_sample_residuals(y_true=y_true, y_pred=y_pred)


def test_set_out_sample_residuals_ValueError_when_y_true_and_y_pred_are_series_with_different_indexes():
    """
...
def test_set_out_sample_residuals_when_residuals_length_is_less_than_10000_and_append():
    ...
    results = np.sort(forecaster.out_sample_residuals_)
    residuals = np.sort(y_true - y_pred)
...
def test_forecaster_set_out_sample_residuals_when_transformer_y_and_differentiation():
    ...
    np.testing.assert_array_almost_equal(forecaster.out_sample_residuals_, residuals)
